...
exports.buildDesignSpec = buildDesignSpec;
function buildVariablesSection(codeModel) {
    const collections = [
...
function buildComponentsSection(codeModel, config) {
    const components = [];
    const mapping = {};
    codeModel.components.forEach((component, index) => {
        const id = `component-${index}`;
        const category = component.kind === 'primitive' ? 'primitive' : 'pattern';
        const pageName = category === 'primitive'
...
function buildScreensSection(codeModel, screensPageName) {
    const screens = [];
    const mapping = {};
...
function buildPagesSection(config) {
    return [
        {
...
function buildDesignSpec(codeModel, config) {
    const variables = buildVariablesSection(codeModel);
    const { components, mapping: componentMapping } = buildComponentsSection(codeModel, config);
    const { screens, mapping: screenMapping } = buildScreensSection(codeModel, config.figma.pages.screens);
