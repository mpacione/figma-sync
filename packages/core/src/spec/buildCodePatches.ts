...
import type {
  ...
  CodePatch,
  CodePatchHunk,
} from '../models/CodePatch';

export function buildCodePatchesForChanges(
  codeModel: CodeModel,
  designSpec: DesignSpec,
  changes: FigmaChangeSet,
): CodePatchSet {
  ...
  for (const change of changes.changes) {
    switch (change.type) {
      case 'RenameComponent': {
        ...
        const codeComponentNames = Object.entries(
          designSpec.mapping.codeComponentToDesignId,
        )
        ...
        const hunks: CodePatchHunk[] = [];

        for (const codeName of codeComponentNames) {
          const matchingComponents = codeModel.components.filter(
            (component) => component.name === codeName,
          );
      ...
      case 'UpdateVariable': {
        ...
        const tokenNames = Object.entries(
          designSpec.mapping.codeTokenToVariableId,
        )
        ...
        for (const tokenName of tokenNames) {
          const colorToken = codeModel.tokens.colors.find(
            (t) => t.name === tokenName,
          );
          ...
          const radiusToken = codeModel.tokens.radii.find(
            (t) => t.name === tokenName,
          );
          ...
          const spacingToken = codeModel.tokens.spacing.find(
            (t) => t.name === tokenName,
          );
          ...
          const typographyToken = codeModel.tokens.typography.find(
            (t) => t.name === tokenName,
          );
