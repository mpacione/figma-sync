...
describe('extractComponentsFromSource', () => {
  it('extracts exported components with kinds and tailwind classes', () => {
    ...
    const components = extractComponentsFromSource(
      src,
    ...
    expect(button.tailwindClasses).toEqual(
      expect.arrayContaining(['px-2', 'text-red-500']),
    );
  });
  ...
  it('ignores non-exported components and non-component exports', () => {
    const src = `function InternalButton() { return <button />; }\nconst internal = () => <div />;\nconst lower = () => <div />;`;
