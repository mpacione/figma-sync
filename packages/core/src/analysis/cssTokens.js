...
exports.extractCssColorTokens = extractCssColorTokens;
exports.extractCssDesignTokens = extractCssDesignTokens;
...
function computePosition(text, index) {
...
function extractCssColorTokens(source) {
    const tokens = [];
...
function extractCssDesignTokens(source) {
    const radii = [];
    const spacing = [];
    const typography = [];
    const text = source.content;
    ...
    while ((match = CSS_VAR_DECL_RE.exec(text)) !== null) {
        ...
        const isRadius = lowerName.includes('radius') || lowerName.includes('round');
        const isSpacing = lowerName.includes('space') ||
            lowerName.includes('spacing') ||
            lowerName.includes('gap') ||
            lowerName.includes('padding') ||
            lowerName.includes('margin');
        const isFontSize = lowerName.includes('font-size') || lowerName.startsWith('text-');
        if (isRadius) {
            radii.push({
                name,
                value: numeric.value,
